...
from . import BasePipeline, ModelConfig, PipelineUnit, PipelineUnitRunner
from .models import ModelManager
from .models.utils import load_state_dict
from .models.wan_video_dit import WanModel, RMSNorm, sinusoidal_embedding_1d
from .models.wan_video_dit_s2v import rope_precompute
from .models.wan_video_text_encoder import WanTextEncoder, T5RelativeEmbedding, T5LayerNorm
from .models.wan_video_vae import WanVideoVAE, RMS_norm, CausalConv3d, Upsample #, ResidualBlock, AttentionBlock
...
class WanVideoPipeline(BasePipeline):
    ...
    def training_loss(self, **inputs):
        forced_timestep_idx = inputs.pop("forced_timestep_idx", None)
        self._last_diffusion_loss = None
        self._last_weighted_diffusion_loss = None
        max_timestep_boundary = int(inputs.get("max_timestep_boundary", 1) * self.scheduler.num_train_timesteps)
        min_timestep_boundary = int(inputs.get("min_timestep_boundary", 0) * self.scheduler.num_train_timesteps)
        if max_timestep_boundary <= min_timestep_boundary:
            max_timestep_boundary = min_timestep_boundary + 1
        timestep_candidates = torch.arange(min_timestep_boundary, max_timestep_boundary, dtype=torch.long)
        if forced_timestep_idx is not None:
            timestep_idx = int(forced_timestep_idx)
            # timestep_idx = max(0, min(int(self.scheduler.timesteps.shape[0]) - 1, timestep_idx))
            timestep_scalar = self.scheduler.timesteps[timestep_idx]
        else:
            rand_idx = torch.randint(0, timestep_candidates.numel(), (1,))
            timestep_id = timestep_candidates[rand_idx]
            timestep_idx = int(timestep_id.item())
            timestep_scalar = self.scheduler.timesteps[timestep_idx]
        timestep = timestep_scalar.reshape(1).to(dtype=self.torch_dtype, device=self.device)
        # print(f"timestep selected: {timestep_idx}, {timestep_scalar.item():.4f}")
        ...
        diffusion_loss = torch.nn.functional.mse_loss(noise_pred.float(), training_target.float())
        diffusion_loss_weight = self.scheduler.training_weight(timestep) * self.diffusion_loss_weight
        total_loss = diffusion_loss * diffusion_loss_weight
        self._last_diffusion_loss = diffusion_loss.detach()
        self._last_weighted_diffusion_loss = total_loss.detach()
    ...
    def __call__(
        ...
        timestep_length = self.scheduler.timesteps.shape[0]
        for progress_id in progress_bar_cmd(range(timestep_length)):
            timestep = self.scheduler.timesteps[progress_id]
            timestep_value = timestep.item()

            # Switch DiT if necessary
            if timestep_value < switch_DiT_boundary * self.scheduler.num_train_timesteps and self.dit2 is not None and not models["dit"] is self.dit2:
            ...
            # Inference
            noise_pred_posi = self.model_fn(**models, **inputs_shared, **inputs_posi, timestep=timestep_tensor)
            if cfg_scale != 1.0:
                if cfg_merge:
                    noise_pred_posi, noise_pred_nega = noise_pred_posi.chunk(2, dim=0)
                else:
                    noise_pred_nega = self.model_fn(**models, **inputs_shared, **inputs_nega, timestep=timestep_tensor)
                noise_pred = noise_pred_nega + cfg_scale * (noise_pred_posi - noise_pred_nega)
            else:
                noise_pred = noise_pred_posi

            latents_clean = self.scheduler.step(noise_pred, self.scheduler.timesteps[progress_id], inputs_shared["latents"], to_final=True)
...
class WanVideoUnit_InputVideoEmbedder(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("input_video", "noise", "tiled", "tile_size", "tile_stride", "vace_reference_image", "precompute_latents"),
            onload_model_names=("vae",)
        )

    def process(self, pipe: WanVideoPipeline, input_video, noise, tiled, tile_size, tile_stride, vace_reference_image,precompute_latents=None):
        if precompute_latents is not None:
            input_latents = precompute_latents
        else:
