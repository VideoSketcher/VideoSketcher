...
from autoregressive.causvid.models import (
...
class InferencePipeline(torch.nn.Module):
    def __init__(self, args, device):
        ...
        self.num_frame_per_block = getattr(
            args, "num_frame_per_block", 3) # it's 3 by default
    ...
    def inference(
        ...
        if start_latents is not None:
            print(start_latents.shape, "start_latents.shape")
            assert start_latents.shape[1] % self.num_frame_per_block == 0
        else:
            print("start_latents is None")
        print(output.shape, "batch_size x num_frames x num_channels x height x width")
        ...
        print("num_input_blocks", num_input_blocks)
        ...
        for block_index in range(num_blocks):
            ...
            absolute_block_index = start_frame_index + block_index * self.num_frame_per_block
            current_start = absolute_block_index * self.frame_seq_length
            ...
            if start_latents is not None and block_index < num_input_blocks:
                ...
                self.generator(
                    ...
                    current_end=current_start + self.num_frame_per_block * self.frame_seq_length
                )
                continue
            ...
            for index, current_timestep in enumerate(self.denoising_step_list):
                ...
                print(f"1. time on index :{time.time() - time_index}")
            ...
            if block_index < num_blocks - 1:  # no need to update cache for the last block
                self.generator(
                ...
                print(f"step 2 cache update :{time.time() - time_b}")

        # Step 3: Decode the output
        time_c = time.time()
        video = self.vae.decode_to_pixel(output)
        video = (video * 0.5 + 0.5).clamp(0, 1) # (batch_size, num_frames, num_channels, height, width)
        # video = video[:,:-4,:,:,:]
        print(f"3. vae decode :{time.time() - time_c}")
        print(f"4. time all :{time.time() - time_global}")
